...
The depth comes from the RabbitMQ management API (`messages_ready` per queue, summed over the connection's planned queues) and requires `queue.connections.<name>.management_url` (+ credentials). Without it, or while a request fails, the depth is unknown and that connection silently keeps a static fleet — no warning, because the API is optional. The supervisor process itself never opens an AMQP connection: the depth sample is an HTTP call, and children forked from the supervisor must not share its sockets (they are not fork-safe).
