...
- The default prefetch rises from 64 to 1000: quorum-queue workers measured ~19-20k → ~47k msg/s consume throughput on the worker path between the two settings. The published config documents the adaptive 250→2000 profile as the recommended high-throughput opt-in (set `prefetch` to the array form or a `RABBIT_RS_PREFETCH` JSON string); adaptive requires acknowledgements — the compiler rejects it with `early_ack` or `no_ack`, which is why it is not the default.
- The extension constraint pins `ext-rabbit_rs` to `^0.3.6`.
