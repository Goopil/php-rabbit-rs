...
- `scripts/lib-extension.sh` provides shared helpers:
  - `ext_artifact_path()`: resolves `target/debug/librabbit_rs_php.{dylib|so}`, building it in debug mode when missing; it only falls back to `target/release/` after printing a readable staleness warning (never silently).
