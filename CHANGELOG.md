...
- The doctor's dead-letter canary no longer false-fails behind a dead-letter backlog (#275): verification pulled one message at the DLQ head with `ack_requeue_true`, never advanced past foreign traffic, and reported "not received" whenever the backlog outlived the probe's patience — healthy dead-letter wiring reported as failed. The probe now scans a bounded window per poll (`CANARY_DLQ_SCAN_WINDOW`, default 100) and classifies by coverage: the probe found in the window = delivered; the whole DLQ visible and the probe absent after the poll budget = genuine failure; a full window (scan evidence inconclusive) or competing consumers on the connection (Horizon running) = `CanaryInconclusiveException` warning instead of a fail. Live-validated behind a 63-message backlog: three connections ok, the Horizon-starved one warn.
...
- Delay modes now refuse or degrade correctly when the broker lacks the `rabbitmq_delayed_message_exchange` plugin. `delay.mode=auto` verifies the plugin through the management API at connection compile time (once per connection per process, only with a `management_url` configured) and degrades to the `ttl` bucket queues when it is absent or unverifiable — deferred jobs no longer publish into the main queue where they could run early (the routing rode the native plugin strategy unconditionally). `delay.mode=plugin` throws the new `DelayPluginMissingException` on the first delayed publish when the management API proves the plugin absent — instead of silently losing every deferred message — and publishes through with a one-time warning when the plugin state cannot be verified, so an unrelated management outage never breaks a working plugin setup. Both probes are cached per connection for the process lifetime (failed probes included, so an unreachable management API never becomes a per-publish timeout).
...
- Auto-scaling for `rabbit-rs:work` (#257): the supervisor can grow and shrink its fleet per connection, driven by the broker's ready depth — from the RabbitMQ management API (`messages_ready`, via the new `queue.connections.<name>.management_url` sampler) when configured, otherwise through a passive native probe (`Pool::size()`, no management plugin needed; unreadable depth silently keeps a static fleet either way). `--min-workers`/`--max-workers` bound the policy (scale up when depth outpaces the live fleet, scale down after 30 s of continuous empty depth, 3 s cooldown, 2 children per pass, clamped to the bounds; long-running downscale signals the idlest children with a non-blocking `SIGTERM` escalated to `SIGKILL` after 15 s, and a released slot never consumes the crash budget). One-shot modes: `--once` and `--stop-when-empty` supervise a fleet that exits after draining (children never recycled; a final depth check re-arms the initial fleet while the broker still reports work, bounded to 3 re-arms), making CI smoke tests and cron drains first-class. `rabbit-rs:doctor` reports the per-supervisor AMQP connection math (workers × brokers). Defaults are unchanged: without the new flags the command behaves exactly as before.

### Docs

- Measured the lone-publish flush-timer latency against the 1 ms contract on main (issue #255): p99 ≤ 6.4 ms, max 7.3 ms across 630 lone publishes (cold, warm, and under concurrent suite load; safe and blind) — and documented why the v0.2.2 report's multi-second tail was a management-API sampled-depth artifact, not timer behavior (`docs/reference.md`, `benchmarks/results/flush-timer-latency/`).
