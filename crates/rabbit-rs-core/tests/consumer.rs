...
async fn partial_consumer_spawn_closes_all_open_channels() {
    ...
    ConsumerSet::spawn_with_metrics(vec![first, second], Metrics::default())
        .await
...
async fn total_prefetch_does_not_overflow_u16() {
    ...
    let consumer = ConsumerSet::spawn_with_metrics(subs, Metrics::default()).await;
...
async fn profile_requested_after_a_publishing_phase_is_established_on_demand() {
    ...
    pool.publish_batch(vec![("b".into(), request)])
        .await
        .expect("publish");
    tokio::time::advance(Duration::from_millis(10)).await;
    tokio::task::yield_now().await;
